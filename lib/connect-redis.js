...
module.exports = function(connect){
  ...
  RedisStore.prototype.publish = function(sid, sess, fn) {
    var self = this;
    if (self.lastPublish && self.lastPublish.sid === sid) {
      if (sess === self.lastPublish.sess) {
        console.log('Avoided multi-publish');
        fn && fn();
        return;
      }
    }

    self.client.publish(sid, sess, function() {
      console.log('publishing', sid, sess);
      self.lastPublish = {sid: sid, sess: sess};
      fn && fn.apply(this, arguments)
    });
  };
