...
module.exports = function(connect){
  ...
  function RedisStore(options) {
    ...
    subClient.on('message', function(channel, message) {
      if (subscriptions[channel] && subscriptions[channel].length) {
        for (var n = 0; n < subscriptions[channel].length; n++) {
          var cb = subscriptions[channel][n];
  ...
  RedisStore.prototype.set = function(sid, sess, fn){
    ...
    try {
      ...
      this.client.setex(sid, ttl, sess, function(err){
        ...
        this.client.publish(sid, session);
